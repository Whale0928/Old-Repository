...
//		
	...
	// - 웹소켓  == 웹소켓에 연결된 클라이언트 세션
	//  -> 세션을 통해서 누가 연결했는지 알 수 있다!

	// -- WebSocketSession을 모아둔다 ??
	// -- 현재 웹소켓에 연결되어 있는 모든 클라이언트를 알 수 있다.
	// --> Set을 분석해서 원하는 클라이언트를 찾아 메세지(채팅)을 전달할 수 있다.
	@Autowired
	private ChatService service;
	
	private Set<WebSocketSession> sessions = Collections.synchronizedSet(new HashSet<WebSocketSession>());
	
	//Collections.synchronizedSet : 동기화된 셋
	//synchronizedSet : 동기화된 셋
	//-> 멀티쓰레드 환경에서 하나의 컬렉션 요소에 여러 스레드가 접근하면 충돌이 발생 할 수 있음으로 동기화를 진행
	// -> 충돌이 발생하지 않도록 줄을 세운다 
		
	@Override //클라이언트와 연결이 완료되고, 통신할 준비가 되면 실행
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		
		
		//WebSocketSession : 웹소켓에 접속 / 요청한 클라이언트 세션
		System.out.println(session.getId()+"연결됨");//세션아이디.
		
		sessions.add(session);
		//WebsocketSessoin을 Set에 추가한다.
		
	}

	@Override//클라이언트로부터 텍스트 메세지를 받았을때 실행	
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
			
		//TextMessage : 웹소켓을 이용해 텍스트로 전달된 메세지가 담겨 있는 객체
		
		//payload : 전송되는 데이터. == JSON
		System.out.println("전달된 메세지 : "+message.getPayload() );
		
		// jackson 라이브러리 : Java에서 JSON을 다루기 위한 라이브러리.

		// jackson-databind 라이브러리 :
		// ObjectMapper 객체를 이용해.
		// JSON 데이터를 특정 VO에 필드에 맞게 자동으로 매핑해준다.
		
		ObjectMapper objectMapper = new ObjectMapper();
		
		ChatMessage chatMessage = objectMapper.readValue(message.getPayload(),ChatMessage.class);
		// JSON 구조를 해재한다 == 각각에 맞게 매핑해준다.
		...
		if(result> 0){
			//같은 방에 접속 중인 클라이언트에게만 메세지를 보내서 출력함.
			// -> 걸러내는 작업 Set<WebSocketSession> 같은 방 클라이언트만 골라내기
			
			for(WebSocketSession s:sessions){ //WebSocketSession == HTTPSession ( 로그인 정보 , 채팅방 번호)를 가로챈것
				
				int chatRoomNo = (Integer)(s.getAttributes().get("chatRoomNo"));
				
				
				//메세지에 담겨잇는 채팅방 번호와 웹소켓 세션에 담겨있는 채팅방 번호가 같을 경우.
				if(chatRoomNo == chatMessage.getChatRoomNo()) {
					//같은방 클라이언트들에게 JSON 형식 메세지를 보냄.
